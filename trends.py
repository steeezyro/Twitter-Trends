...
def analyze_tweet_sentiment(tweet):
    ...
    for w in words:
        ...
        if has_sentiment(sentiment):
            count += 1
            sent += sentiment_value(sentiment)
    if count == 0:
        return average
    else:
        return make_sentiment(sent / count)
...
def find_state_center(polygons):
    ...
    """
    "*** YOUR CODE HERE ***"
    sum_num_x, sum_num_y, sum_denom = 0, 0, 0
    for p in polygons:
        t = find_centroid(p)
        sum_num_x += t[0] * t[2]
        sum_num_y += t[1] * t[2]
    ...
    x = sum_num_x / sum_denom
    y = sum_num_y / sum_denom
...
def group_by_key(pairs):
    """This function returns a dictionary that relates each unique key in [key, value] pairs
    to a list of all values that are paired with that key.
    """
...
def group_tweets_by_state(tweets):
    ...
    for tweet in tweets:
        nearest_state = add_nearest_state(tweet)[0]  
